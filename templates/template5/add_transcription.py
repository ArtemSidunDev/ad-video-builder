...
def create_caption(
    ...
    start_y_pos = int( (frame_height - total_text_height) * 0.82)
    ...
    for word_info in xy_textclips_positions:
        ...
        outline_text_clips = generate_outline_text(
            text=word_info['word'],
            font=font,
            fontsize=fontsize,
            color=color,
            outline_color=FONT_OUTLINE_COLOR,
            pos=(centered_x_pos, start_y_pos + word_info['y_pos']),
            duration=full_duration,
            start=textJSON['start']
        )
        word_clips.extend(outline_text_clips)
...
click_audio = AudioFileClip("./templates/template5/input/mouse-click.mp3")
...
audios.append(click_audio.set_start(19.08))
audios.append(click_audio.set_start(21.96))
