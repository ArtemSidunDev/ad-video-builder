...
FONT = "./input/ProximaNova-Black.ttf"
FONT_SIZE = 80
FONT_COLOR = "#FFFFFF"
...
foreground_audio = VideoFileClip("./input/avatar.mp4").speedx(1.04).audio
...
with open('./input/transcription.json', 'r') as f:
...
def create_caption(
    ...
    start_y_pos = (frame_height - total_text_height) / 2
...
camera_audio = AudioFileClip("./input/camera.mp3").subclip(0.4, 0.8).fx(afx.audio_normalize).fx(afx.volumex, 0.5)
transition_audio = AudioFileClip("./input/transition.wav").subclip(0, 0.4).fx(afx.audio_normalize).fx(afx.volumex, 0.3)

audios = [foreground_audio.fx( afx.audio_normalize).fx(afx.volumex, 1.3)]

audios.append( VideoFileClip("./input/background_sound.mp4").audio.fx(
        afx.audio_normalize).fx( afx.volumex, 0.3).set_duration(35))
...
audios.append(
    VideoFileClip("./input/action.mp4").audio.fx(
